require 'rails_helper'

# This spec was generated by rspec-rails when you ran the scaffold generator.
# It demonstrates how one might use RSpec to test the controller code that
# was generated by Rails when you ran the scaffold generator.
#
# It assumes that the implementation code is generated by the rails scaffold
# generator. If you are using any extension libraries to generate different
# controller code, this generated spec may or may not pass.
#
# It only uses APIs available in rails and/or rspec-rails. There are a number
# of tools you can use to make these specs even more expressive, but we're
# sticking to rails and rspec-rails APIs to keep things simple and stable.

RSpec.describe "/designations", type: :request do
  
  # Designation. As you add validations to Designation, be sure to
  # adjust the attributes here as well.
  let(:valid_attributes) {
    skip("Add a hash of attributes valid for your model")
  }

  let(:invalid_attributes) {
    skip("Add a hash of attributes invalid for your model")
  }

  describe "GET /index" do
    it "renders a successful response" do
      Designation.create! valid_attributes
      get designations_url
      expect(response).to be_successful
    end
  end

  describe "GET /show" do
    it "renders a successful response" do
      designation = Designation.create! valid_attributes
      get designation_url(designation)
      expect(response).to be_successful
    end
  end

  describe "GET /new" do
    it "renders a successful response" do
      get new_designation_url
      expect(response).to be_successful
    end
  end

  describe "GET /edit" do
    it "render a successful response" do
      designation = Designation.create! valid_attributes
      get edit_designation_url(designation)
      expect(response).to be_successful
    end
  end

  describe "POST /create" do
    context "with valid parameters" do
      it "creates a new Designation" do
        expect {
          post designations_url, params: { designation: valid_attributes }
        }.to change(Designation, :count).by(1)
      end

      it "redirects to the created designation" do
        post designations_url, params: { designation: valid_attributes }
        expect(response).to redirect_to(designation_url(Designation.last))
      end
    end

    context "with invalid parameters" do
      it "does not create a new Designation" do
        expect {
          post designations_url, params: { designation: invalid_attributes }
        }.to change(Designation, :count).by(0)
      end

      it "renders a successful response (i.e. to display the 'new' template)" do
        post designations_url, params: { designation: invalid_attributes }
        expect(response).to be_successful
      end
    end
  end

  describe "PATCH /update" do
    context "with valid parameters" do
      let(:new_attributes) {
        skip("Add a hash of attributes valid for your model")
      }

      it "updates the requested designation" do
        designation = Designation.create! valid_attributes
        patch designation_url(designation), params: { designation: new_attributes }
        designation.reload
        skip("Add assertions for updated state")
      end

      it "redirects to the designation" do
        designation = Designation.create! valid_attributes
        patch designation_url(designation), params: { designation: new_attributes }
        designation.reload
        expect(response).to redirect_to(designation_url(designation))
      end
    end

    context "with invalid parameters" do
      it "renders a successful response (i.e. to display the 'edit' template)" do
        designation = Designation.create! valid_attributes
        patch designation_url(designation), params: { designation: invalid_attributes }
        expect(response).to be_successful
      end
    end
  end

  describe "DELETE /destroy" do
    it "destroys the requested designation" do
      designation = Designation.create! valid_attributes
      expect {
        delete designation_url(designation)
      }.to change(Designation, :count).by(-1)
    end

    it "redirects to the designations list" do
      designation = Designation.create! valid_attributes
      delete designation_url(designation)
      expect(response).to redirect_to(designations_url)
    end
  end
end
