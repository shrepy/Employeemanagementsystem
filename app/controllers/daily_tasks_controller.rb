...
class DailyTasksController < ApplicationController
  ...
  def new
    task = current_employee.daily_tasks.where('EXTRACT(DAY FROM created_at) = ? AND EXTRACT(MONTH FROM created_at) = ? AND EXTRACT(YEAR FROM created_at) = ?', Date.current.day, Date.today.month, Date.today.year).first
    if task.blank?
  ...
  def edit
    @daily_task = current_employee.daily_tasks.where('EXTRACT(DAY FROM created_at) = ? AND EXTRACT(MONTH FROM created_at) = ? AND EXTRACT(YEAR FROM created_at) = ?', Date.current.day, Date.today.month, Date.today.year).first
